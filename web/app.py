...
def gacha_pull():
    ...
    # 2. GESTION DU TIRAGE DE RACE
        ...
        db.users.update_one(
            {"user_id": request.user_id},
            {
                "$inc": {"tickets_race": -1, "gacha_total_pulls": 1},
                "$set": {"current_race": recompense},
                "$addToSet": {"unlocked_races": recompense}
            }
        )
    else:
    ...
    return jsonify({
        "success": True,
        "reward": recompense,
        "tickets_classe": new_doc.get("tickets_classe", 0),
        "tickets_race": new_doc.get("tickets_race", 0),
